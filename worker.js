  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (
        ...
        /*
          Цена продажи = 50% от цены предмета.
          Позже сделаем процент настраиваемым
          через админ-панель.
        */
      ...
      if (
        ...
        if (existing) {
          return json({
            ok: false,
            error: "Реферал уже зарегистрирован"
          }, headers, 409);
        }
        ...
        /*
          ...
          реферальную награду через админку.
        */
      ...
      if (url.pathname.startsWith("/api/admin/")) {
        ...
        if (
          ...
          await env.DB.prepare(`
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
          `).bind(
            String(body.title || "Задание"),
            String(body.description || ""),
            String(body.type || "link"),
            body.url || null,
            Math.max(0, Number(body.reward || 0)),
            Number(body.is_active ?? 1),
            body.max_completions == null
              ? null
              : Number(body.max_completions)
          ).run();
